...
@Component({
    selector: 'app-claims-completados',
    standalone: true,
    imports: [CommonModule, RouterModule, FormsModule],
    templateUrl: './completados.component.html'
})
export class ClaimsCompletadosComponent implements OnInit, OnDestroy {
    claims: Claim[] = [];
    ...
    loadClaims(): void {
        ...
        this.subscription = this.claimsService.getAllClaims().subscribe({
            next: (claims) => {
                this.claims = claims.filter(c => c.status === 'COMPLETADO');
                this.applyFilter();
                this.isLoading = false;
            },
            error: () => {
                this.error = 'Error al cargar los reclamos completados.';
                this.isLoading = false;
            }
        });
    }
