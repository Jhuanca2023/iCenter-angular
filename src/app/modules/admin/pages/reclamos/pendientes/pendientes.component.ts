...
@Component({
    selector: 'app-claims-pendientes',
    standalone: true,
    imports: [CommonModule, RouterModule, FormsModule],
    templateUrl: './pendientes.component.html'
})
export class ClaimsPendientesComponent implements OnInit, OnDestroy {
    claims: Claim[] = [];
    ...
    loadClaims(): void {
        ...
        this.subscription = this.claimsService.getAllClaims().subscribe({
            next: (claims) => {
                this.claims = claims.filter(c => c.status === 'PENDIENTE');
                this.applyFilter();
                this.isLoading = false;
            },
            error: () => {
                this.error = 'Error al cargar los reclamos pendientes.';
                this.isLoading = false;
            }
        });
    }
